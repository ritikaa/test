...
var single = '';
	...
	for(i = 0; i < input.length; i++) 
	{
		if(i == 0)
		{
			single = input.charAt(0);
		}
		else if(all_same && single != input.charAt(i))
		{
			all_same = false;
		}
		//console.log("checking " + input.charAt(i));
...
if(all_same)
	{
		document.getElementById("samepinError").style.display="inline";
		return false;
	}
...
var mname=document.getElementById("umname").value;

for(i = 0; i < mname.length; i++) 
	{
		//console.log("checking " + input.charAt(i));
		
		if(!((mname.charAt(i) >= 'A' && mname.charAt(i) <= 'Z') || (mname.charAt(i) >= 'a' && mname.charAt(i) <= 'z')))
		{
...
for(i = 0; i < input1.length; i++) 
	{
		...
		if(!(input1.substring(input1.length-17) == '@softageindia.com'))
		{
...
var empc22 = document.getElementById('emp_code');
var empcodfiltewr =/^[S-s][0-9]*$/;
if (!empcodfiltewr.test(empc22.value)) {
hideAllErrors();
...
document.getElementById("samepinError").style.display="none";
